...
export class Font {
    ...
    public toCss() {
        let _css = {'font-family': '', 'font-size': '', 'font-weight': '',
                    'font-style': '', 'text-decoration': '', 'width': 0};
    ...
    public static parseFont(pzFont: string, defaultFont?: Font) {
        ...
        let _zFontName: string = defaultFont ? defaultFont.FontFamily : BravoSettings.current.zDefaultFontName,
            _fontSize: any = defaultFont ? defaultFont.nSize : BravoSettings.current.nFontSize,
            _fontStyle: FontStyle = FontStyle.Regular;

        if (_zStyleFont.length >= 3) {
            if (_zStyleFont[0])
                _zFontName = _zStyleFont[0];

            if (_zStyleFont[1])
                _fontSize = _zStyleFont[1];
