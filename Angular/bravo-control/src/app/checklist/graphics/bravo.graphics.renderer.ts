...
export class BravoGraphicsRenderer {
    ...
    public static measureString(pzText: string, font: Font, pnWidth?: number, pBreakWords: boolean = true) {
        ...
        let _css = font.toCss(),
            _textStyle = {'fontFamily': '', 'fontWeight': '', 'fontSize': '', 'fontStyle': '',
                          'wordWrapWidth': 0, 'breakWords': true, 'width': 0},
            _wordWrap = false;
        ...
        if (Object.keys(_css).length > 0) {
            ...
            if (!BravoGraphicsRenderer.measureCache.has(_zStyleName)) {
                ...
                BravoGraphicsRenderer.measureCache.set(_zStyleName, new TextStyle(_textStyle));
            }

            return BravoTextMetrics.measureText(pzText, BravoGraphicsRenderer.measureCache.get(_zStyleName)!,
                _wordWrap, document.createElement('canvas'));
        }
    }
