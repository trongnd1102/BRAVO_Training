...
export function isIgnored(item: any) {
    if (item instanceof wjg.Column)
        return item.mask == IgnoreHeaderEditmask;

    if (item instanceof wjg.Row)
        return item['_mask'] == IgnoreHeaderEditmask;
...
export function getControl(element: any) {
    ...
    while (element instanceof HTMLElement && element.parentElement) {
        ...
        if (_ctrl && !_ctrl.isDisabled && element instanceof HTMLElement &&
...
export function getActiveControl() {
    ...
    let _hostCtrl = wjc.closestClass(_activeElement, 'bravo-control') ||
...
export function isCellValid(pPanel: wjg.GridPanel, pnRow: number, pnCol: number) {
    return !isCellInValid(pPanel, pnRow, pnCol);
}

export function isCellInValid(pPanel: wjg.GridPanel, pnRow: number, pnCol: number) {
...
class CharacterRange {
    ...
    public equals(obj: any) {
        if (this.first == obj.first && this.length == obj.length)
            return true;
