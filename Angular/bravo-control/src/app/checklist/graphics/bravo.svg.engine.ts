...
export class BravoSvgEngine extends wjChart._SvgRenderEngine {
    ...
    public static getPostionFillText(pElement: Element, pzAlign: string) {
        let _attribute = {'x': '', 'y': '', 'text-anchor': '', 'dominant-baseline': ''};
...
export class BravoSvgEngineHelper {
    ...
    public static saveImageToDataUrl(pzFormat: 'png' | 'jpeg' | 'svg', element: any, option?: { width: number, height: number }): Observable<any> {
        if (!String.isNullOrEmpty(pzFormat)) {
            return new Observable((observer) => {
                BravoSvgEngineHelper.exportToImage(pzFormat, element, option, (dataURI: any) => {
    ...
    public static exportToImage(extension: string, element: any, options: { width: number, height: number }, processDataURI: { (dataURI: any): void; call?: any; }) {
        let image = new Image(),
            ele = element,
            dataUrl;
    ...
    //clone from class ExportHelper in wijmo.chart
    public static getDataUri(ele: any) {
    ...
    //clone from class ExportHelper in wijmo.chart
    public static getStyles(ele: any) {
        var css = '',
